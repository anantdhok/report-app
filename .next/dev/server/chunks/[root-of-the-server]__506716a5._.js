...
"[project]/src/app/api/calls-export/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
        ...
        const lines = [
            ...
            ...callRecords.map((c)=>{
                const vals = [
                    ...
                    c.calleeNumber || "",
                    c.callType?.[0]?.value || "",
                    Array.isArray(c.category) ? c.category.map((x)=>x.value || x).join("|") : "",
                    c.criticality?.[0]?.value || "",
                    c.fileName || ""
                ];
                return vals.map((v)=>`"${String(v).replace(/"/g, '""').replace(/\r?\n/g, " ")}"`).join(",");
            })
        ];
...
//# sourceMappingURL=%5Broot-of-the-server%5D__506716a5._.js.map