...
export async function POST(req: Request) {
  try {
    ...
    const payload = raw[body?.targetId]?.data?.data || {};
    ...
    const targetDerived =
      ...
      (firstCall
        ? {
            ...
            code:
              payload.targetCode || `TGT${firstCall.callerNumber || firstCall.calleeNumber || ""}`,
            targetNumber:
              payload.targetNumber || firstCall.callerNumber || firstCall.calleeNumber || "",
            ...
            criticalFileCount:
              payload.criticalFileCount ?? payload.reportMetadata?.criticalKeywords ?? 0,
            filteredFileCount: payload.filteredFileCount ?? calls.length ?? 0,
            filteredCriticalFileCount:
              payload.filteredCriticalFileCount ??
              payload.reportMetadata?.highPriorityKeywords ??
              0,
    ...
    const core = {
      ...
      matchedTargetKeywords:
        payload.matchedKeywords?.data?.matchedTargetKeywords || payload.matchedTargetKeywords || [],
      matchedGlobalKeywords:
        payload.matchedKeywords?.data?.matchedGlobalKeywords || payload.matchedGlobalKeywords || [],
